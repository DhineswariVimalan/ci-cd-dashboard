...
def main(): 
    ...
    if search_term: 
        filtered_data = data[data.apply(lambda row: 
row.astype(str).str.contains(search_term, case=False).any(), axis=1)] 
    ...
    age_range = st.slider("Select Age Range", min_value=int(data["age"].min()), 
max_value=int(data["age"].max()), value=(30, 60)) 
    filtered_by_age = data[(data["age"] >= age_range[0]) & (data["age"] <= 
age_range[1])] 
